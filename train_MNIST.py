...
class TorchCNN(nn.Module):
    def __init__(self):
        """
        Initialize a ZF network
        Sub layers are accumulated in self.layers
        Divided every filter size by 4 to save computation time
        ...
        # hidden layer 6 (ended here to save parameters)
    ...
    def forward(self, x):
        """Forward pass, will be used for training"""
        i = 0
        for layer in self.layers:
            x = layer(x)
        return x

def transform_data(mnist_train, mnist_test):
    """
    Transforms data:
        - increase image size
        - change to (data, label) pairs for DataLoader
    """
...
def train_MNIST(epochs, train_loader, test_loader):
    """"
    Train ZF network on MNIST dataset

    args :
        epochs : number of epochs
        train_loader : pytorch train data in batches
        test_loader : pytorch test data in batches
    """
...
if __name__ == '__main__':
    ...
    train_data, test_data = transform_data(mnist_train, mnist_test)
    ...
    print(summary(net, (1, 224, 224), device='cpu'))

    train_losses, train_accs, test_accs = train_MNIST(epochs=5, train_loader=train_loader, test_loader=test_loader)
